...
stmt:	expr SEMICOLON		{				if(call_flag ==1){
								...
								func_call_flag=0;
								///print///printf("Expresion\n");
							}
	| ifstmt		{
						///print///printf("IF statement\n");
					}
	| whileall		{
						///print///printf("WHILE statement\n");
					}
	| for			{
						///print///printf("FOR statement\n");
					}
	| returnstmt		{
							if(deepestFunctionScope == 0 ){
										printf("Syntax Error --in line %d :RETURN is declared outside of any function.\n" , yylineno);
							}
							///print///printf("RETURN statement\n");
						} 
	| break SEMICOLON	{
							///print///printf("|BREAK\n");
							if (loopScope==0){
	...
	| funcdef		{
						///print///printf("Function definition\n");
					}
	| SEMICOLON		{
						///print///printf("--> ; <--\n");
					}
...
expr:	assignexpr		{first_flag = 0;/*///print///printf("Assign expression \n");*/resettemp();
				$<exprval>$ = $<exprval>1;

}
	| expr PLUS expr	{///print///printf("expression  + expression\n"); 
				$<exprval>$ = arith_emit(add,$<exprval>1,$<exprval>3,yylineno);	
				}			
	| expr MINUS expr	{///print///printf("expression  - expression\n");
				$<exprval>$ = arith_emit(sub,$<exprval>1,$<exprval>3,yylineno);
				}
	| expr MUL expr		{///print///printf("expression  * expression \n");
				$<exprval>$ = arith_emit(mul,$<exprval>1,$<exprval>3,yylineno);				
				}
	| expr DIV expr		{///print///printf("expression  / expression\n");
				$<exprval>$ = arith_emit(divide,$<exprval>1,$<exprval>3,yylineno);
				}
	| expr MOD expr		{///print///printf("expression %% expression\n");
				$<exprval>$ = arith_emit(mod,$<exprval>1,$<exprval>3,yylineno);
				}
	| expr GRTR expr	{///print///printf("expression  > expression\n");
				$<exprval>$ = emitrelop(if_greater, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr GRTR_EQL expr	{///print///printf("expression >= expression\n");
				$<exprval>$ = emitrelop(if_greatereq, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr LESS expr	{///print///printf("expression  < expression\n");
				$<exprval>$ = emitrelop(if_less, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr LESS_EQL expr	{///print///printf("expression <= expression\n");
				$<exprval>$ = emitrelop(if_lesseq, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr EQL expr		{///print///printf("expression == expression\n");
				$<exprval>$ = emitrelop(if_eq, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr N_EQL expr	{///print///printf("expression != expression\n");
				$<exprval>$ = emitrelop(if_not_eq, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr AND expr		{///print///printf("expression && expression\n");
				$<exprval>$ = emitlogic(and, $<exprval>1, $<exprval>3, yylineno);
				}
	| expr OR expr		{///print///printf("expression || expression\n");
				$<exprval>$ = emitlogic(or, $<exprval>1, $<exprval>3, yylineno);
				}
	| term			{///print///printf("Terminal \n");
				$<exprval>$ = $<exprval>1;	
				}
...
term:	L_PARENTH expr R_PARENTH	{///print///printf("( expression )\n");
					$<exprval>$ = $<exprval>2;
}
	| MINUS expr %prec UMINUS	{///print///printf("- expression\n");
					if(func_call_flag==1){
	...
	| NOT expr			{///print///printf("!expression\n"); 
	...
	| PLUSPLUS lvalue		{///print///printf("++ lvalue\n");
					if(func_call_flag==1){
											printf("Syntax Error: In line %d .Trying to make an lvalue action to function[invalid action : ++rvalue].\n",yylineno);
					func_call_flag=0;
				}	
	...
	| lvalue PLUSPLUS		{///print///printf("lvalue ++\n");
					if(func_call_flag==1){
											printf("Syntax Error: In line %d .Trying to make an lvalue action to function[invalid action : rvalue++].\n",yylineno);
					func_call_flag=0;
					}
	...
	| MINUSMINUS lvalue		{///print///printf("-- lvalue\n");
					if(func_call_flag==1){
											printf("Syntax Error: In line %d .Trying to make an lvalue action to function[invalid action : -rvalue].\n",yylineno);
					func_call_flag=0;
					}
	...
	| lvalue MINUSMINUS		{///print///printf("lvalue --\n");
					if(func_call_flag==1){
											printf("Syntax Error: In line %d .Trying to make an lvalue action to function[invalid action : rvalue--].\n",yylineno);
					func_call_flag=0;
					}
	...
	| primary			{///print///printf("Primary in terminal\n");									
					$<exprval>$ = $<exprval>1;
}
	;
...
assignexpr:	lvalue ASSIGN expr	{
					...
					if(($<exprval>1->type)==tableitem_e){
						...
						//printf("sdfasfgfdgd: %d",$<exprval>1->sym->offset);
						$<exprval>$=newexpr_t(assignexpr_e);
						if(!istemp($<exprval>1->sym)){
							$<exprval>$->type = var_e; //LINE ADDED -----LULUDAK------
							$<exprval>$->sym = newtemp();
							//$<exprval>1->sym->offset = currscopeoffset();
							emit(assign,$<exprval>1,NULL,$<exprval>$,-1,yylineno);
							...
							$<exprval>$->sym = $<exprval>1->sym;
									...
									///print///printf("lvalue = expression \n");
								}
	;
...
primary:	lvalue			{			$<exprval>$ =emit_iftableitem($<exprval>1,yylineno);
	...
	| objectdef			{
							...
							$<exprval>$ = $<exprval>1;
							///print///printf("Object definition\n");
							
						}
	| L_PARENTH funcdef R_PARENTH	{
										///print///printf("(Function definition):%s\n",$<symval>2->type);
										$<exprval>$ = newexpr_t(programfunc_e);
										$<exprval>$->sym = $<symval>2;
										
									}
	| const				{	
							///print///printf("Constant\n");
							$<exprval>$ = $<exprval>1;
						}
	;
...
lvalue:	IDENTIFIER	{
				...
				if(funcdef_flag==1){
					...
					if( lookUpH( (createS( tmp_buf , deepestScope , 0 , "" , deepestFunctionScope, 0 , "var" , 1 ,currscopespace(),currscopeoffset()) ) , "funcvar"  , "illscope")!=NULL ){
						...
						$<exprval>$ = lvalue_expr(lookUpH( (createS( tmp_buf , deepestScope , 0 , "" , deepestFunctionScope, 0 , "var" , 1 ,currscopespace(),currscopeoffset()) ) , "funcvar"  , "illscope"));
		                        }
					...
					//	printf("[Notice:Variable is redefined-refferencing to it.]\n");
						$<exprval>$ = lvalue_expr(lookUpH( (createS( tmp_buf , deepestScope , 0 , "" , deepestFunctionScope, 0 , "var" , 1 ,currscopespace(),currscopeoffset()) ) , "funcvar"  , "sscope"));
						...
						inccurrscopeoffset();
						Symbol * tmpS = createS(tmp_buf,deepestScope,0,topfStackName(),deepestFunctionScope,0,"var",1,currscopespace(),currscopeoffset());
						insertH(tmpS);
						$<exprval>$ = lvalue_expr(tmpS);						
				...
				//$<exprval>$ = lvalue_expr(createS(tmp_buf,0,0,"",0,0,"var",1,currscopespace(),currscopeoffset()));
				///print///printf("CURR SCOPE OFFSET HERE, in value:%s : %d",tmp_buf,currscopeoffset());	//Keno to function scope???? - LULUDAK-	
			} 
	| LOCAL IDENTIFIER 		{
							
							///print///printf("LOCAL METAVLITI:%s\n",(char *) $<exprval>2);
							...
							if(tmp_buf!=NULL)free(tmp_buf);
								...
								Symbol *tmpSym;
								if( (tmpSym=lookUpH( (createS( $2 , 0 , 0 , "" , deepestFunctionScope , 0 , "var" , 1 ,currscopespace(),currscopeoffset()) ) , "funcvar"  , "localscope"))!=NULL ){
									
									///print///printf("Variable found.Refferencing to it.\n");
									$<exprval>$ =  lvalue_expr(tmpSym);
...
//$<exprval>$ = lvalue_expr(createS($2,0,0,"",0,0,"var",1,currscopespace(),currscopeoffset()));
									...
									Symbol * tmpSym = createS(tmp_buf,deepestScope,0,topfStackName(),deepestFunctionScope,0,"var",1,currscopespace(),currscopeoffset());
									isLocalvar=1;	
									inccurrscopeoffset();						
									insertH(tmpSym);
									$<exprval>$ = lvalue_expr(tmpSym);
								...
								///print///printf("CURR SCOPE OFFSET HERE LOCALLY, in value: %d",currscopeoffset());
								///print///printf("|LOCAL ID: %s\n",yytext);
...
member:	lvalue DOT IDENTIFIER			{/*printf("lvalue.ID\n");*/  $<exprval>$=emit_table($<exprval>1,constString_expr(yylval.strval),yylineno);}	 //TODO: xreiazetai: table emit.
	| lvalue L_BRACKET expr R_BRACKET	{///print///printf("lvalue[expr]\n");
...
call:	call L_PARENTH elist R_PARENTH 						{///print///printf("CALL(Elist)\n");
		$<exprval>$ = (expr *) func_call($<exprval>1, $<exprval>3, yylineno);

} 
	| lvalue L_PARENTH elist R_PARENTH	{///print///printf("|lvalue(elist) - Normal Call\n");
						$<exprval>$ = (expr *) func_call($<exprval>1, $<exprval>3, yylineno);
						}
	|lvalue DBL_DOT IDENTIFIER L_PARENTH elist R_PARENTH	{///print///printf("|lvalue..ID(Elist)-Method Call\n");
	...
	| L_PARENTH funcdef R_PARENTH L_PARENTH elist R_PARENTH {///print///printf("(funcdef)(Elist)\n");
	///print///printf("(FUNCDEF)(ELIST)\n");
...
elist:	expr tmp_elist				{///print///printf("Expr\n");
						$<exprval>$ = $<exprval>1;
						$<exprval>$->next = $<exprval>2;
						} 
	|					{///print///printf("Empty Elist\n");
						$<exprval>$ = NULL;						
						}
	;

tmp_elist: COMMA expr tmp_elist			{///print///printf("Expr,....,Expr\n");
						$<exprval>$ = $<exprval>2;
...
objectdef:	L_BRACKET elist R_BRACKET	{///print///printf(" [ Elist ] \n");
...
table->sym = newtemp();
   emit(tablecreate, NULL, NULL, table, -1, yylineno);
	...
	| L_BRACKET indexed R_BRACKET		{///print///printf(" [ Indexed ] \n");
	expr *table= newexpr_t(newtable_e);
	table->sym = newtemp();
	emit(tablecreate,NULL,NULL,table,-1,yylineno);
	expr *tmp = $<exprval>2;
...
indexed: indexedelem tmp_indexed 	{///print///printf("indexedelem\n");
					$<exprval>$ = $<exprval>1;
					$<exprval>$->next = $<exprval>2;			
					}
	;
...
tmp_indexed: COMMA indexedelem tmp_indexed {
					///print///printf("indexedelem , indexedelem\n");					
					$<exprval>$ = $<exprval>2;
					$<exprval>$->next = $<exprval>3; }
	|	{$<exprval>$ = NULL;}
...
indexedelem:	L_BRACE expr COLON expr R_BRACE	{///print///printf("{ Expr : Expr }\n");
		 $<exprval>$ = $<exprval>2;
...
$<symval>$ = expr_tmp->sym;
//Exw metakinisei to pedio me ta loopstack kai ta localargs panw apo to emit
node* tmp = popStack(loopStack);
if (tmp)
	loopScope = tmp->info;
int localArgs = findLocal(tmp_buf->sym->name);
expr_tmp = lvalue_expr(tmpS);
expr_tmp->lvar =localArgs;
emit(funcend,NULL,NULL,expr_tmp,nextquadlabel(),yylineno);

free(tmp_buf);
///print///printf("Funcdef\n");
...
funcprefix:	FUNCTION ident_tmp {
				...
				if( lookUpH( (createS( f_name , 0 , 0 , "" , deepestFunctionScope, 0 , "func" , 1 ,currscopespace(),currscopeoffset()) ) , "funcdef"  , "samescope")!=NULL ){
					...
					///print///printf("EEEEEEEEEEEEEEEEEEEEEEEEERRRRRRRRRRRRRRROR");
...
funcbody:		block {
				...
				deepestFunctionScope--;
				///print///printf("Function block\n");
				restorecurrscopeoffset(popOffsetStack(offsetStack));
				exitscopespace();
			};

const:	INTEGER			{///print///printf("|NUMBER %s\n",yytext);
				$<exprval>$ = constInt_expr(yylval.intval);	
				} 
	| REALNUMBER		{///print///printf("|REALNUMBER\n");
				$<exprval>$ = constDouble_expr(yylval.doubleval);
				}
	| STRING		{///print///printf("|STRING\n");
				$<exprval>$ = constString_expr(yylval.strval);	
				}
	| NIL			{///print///printf("|NIL\n");
				$<exprval>$ = newexpr_t(nil_e);
				}
	| TRUE			{///print///printf("|TRUE\n");
				$<exprval>$ = constBool_expr('1');
				}
	| FALSE			{///print///printf("|FALSE\n");
...
whileall: whilestart whilestmt whilecond whileblock {///print///printf("WHILE (EXPR) STMT\n");
...
forend:		{
			...
			while((tmpstack)&& (tmpstack->jump == tmpquadnum))
			{
				tmpstack = popSpecialJumpStack(breakSpecialStack);
				if (tmpstack)	
					backpatch(tmpstack->start,currQuad);
...
N:	{
		$<intval>$ = currQuad;
		///print///printf("BENEI KAI STO N1: %d \n",$<intval>$);
		emit(jump, (expr*)NULL, (expr*)NULL,  (expr*)NULL, -1, yylineno);
		///print///printf("BENEI KAI STO N2: %d \n",$<intval>$);
	}
;
