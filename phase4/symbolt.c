...
int hashF( char *name )
{
	int i = ((( generateS(name))) % HASH_SIZE);
	return i;
}
...
Symbol *lookUpH( Symbol *S , char *mode , char *crit )
{
	...
	if( tmp == NULL ){
		...
		tmp = hashtable[ k ];
		while( tmp!= NULL ){
			if( (strcmp( S->name , tmp->name ) ) == 0 && tmp->scope == 0 && tmp->active == 1 ){
				return tmp;
			}
			tmp = tmp->next;
		}
		...
		while( tmp != NULL ){
			 if  (!strcmp(tmp->type,"var")){
			 	///print///printf("Exei perasei apo dw gia to testing mas.\n");
				if(strcmp( S->name , tmp->name ) == 0 && tmp->scopeF< S->scopeF && tmp->scope != 0 && tmp->active == 1){
					return tmp;
			   	}
			}
			else if  (!strcmp(tmp->type,"formarg")){
		...
		tmp = hashtable[ k ];
		while( tmp != NULL ){
			if(( strcmp( S->name , tmp->name ) ) == 0 && ( S->scopeF == tmp->scopeF ) && tmp->active == 1){
				return tmp;
			}
			tmp = tmp->next;
		}
		...
		tmp = hashtable[ k ];
		while( tmp != NULL ){
			if(!strcmp(tmp->type,"func") && strcmp( S->name , tmp->name  ) == 0 && ( S->scopeF == tmp->scopeF) && tmp->active == 1){
				return tmp;
			}
			tmp = tmp->next;
		}		
	return NULL;
		...
		tmp = hashtable[ k ];
		while( tmp != NULL ){
			if( ( strcmp( S->name , tmp->name ) ) == 0 && ( tmp->scopeF == 0 ) && tmp->active == 1){
				return tmp;
			}
			tmp = tmp->next;
		}		
		...
		while( tmp != NULL ){
			if( ( strcmp( S->name , tmp->name ) ) == 0 &&
				( S->scopeF == tmp->scopeF ) && ( tmp->active == 1 )&& tmp->active == 1 ){
				return tmp;
			}
			tmp = tmp->next;
		}
...
//na ta valw edw.
	...
	if( ins_lib_func == 0 ){
		for( i=0; i<12; i++){	//elegxos an eisagoume variable pou exei to idio onoma me kapoia library function
			if(((strcmp( newS->type , "var")==0) || (strcmp( newS->type , "func")==0) ||
				...
				//printf("Notice: Den ginetai eisagogi var i function pou na exei idio onoma me mia library function\n");
				return 0;
			}
		}
		...
		if( tmp != NULL ){					
			if( tmp->scopeF == newS->scopeF && tmp->active == 1 ){
				//printf("Notice: Den ginetai eisagogi var pou na exei idio onoma me funct sto idio scope.\n");
				return 0;
			}
		}
		...
		if (isLocalvar!=1 && strcmp(newS->type,"formarg")!=0 && strcmp(newS->type,"func")!=0){
			if( tmp != NULL ){
				//printf("Notice: Den ginetai eisagogi var pou na exei idio onoma me mia global/local metavliti\n");
				return 0;
			}
		}
		...
		if( tmp != NULL ){
			//printf("Notice: Den ginetai eisagogi var pou na exei idio onoma me mia var pou einai se mikrotero function scope ( mesolavei function )\n");
			return 0;
		}
		tmp = lookUpH( newS , "funcvar" , "scope" ); //elegxos gia dilosi function i variable me to idio onoma kai sto idio scope
		if( tmp != NULL ){
			//printf("Notice: Den ginetai eisagogi stoixeiou pou na exei idio onoma me mia function i ena variable sto idio scope\n");
			return 0;
		}
		tmp = lookUpH( newS , "formarg" , "scope" );
		if( tmp != NULL ){
			//printf("Notice: Den ginetai eisagogi formal arguments pou na exei idio onoma me allo formal argument idiou scope kai anikoun stin idia sinartisi\n");
			return 0;
		}
	}
	/*
	...
	*/
	...
	///print///printf("TO OFFSET TIS TMP:-------------%d\n",currscopeoffset());
	i = hashF( newS->name );
	if( hashtable[ i ] == NULL ){
		hashtable[ i ] = nnode;
		inccurrscopeoffset();
